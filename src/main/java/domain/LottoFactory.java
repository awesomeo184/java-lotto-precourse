...
public class LottoFactory {
    ...
    public static void validateNonDuplicateNumbers(List<Integer> numbers) {
        ...
        if (set.size() != NUMBERS_UNIT) {
            throw new IllegalArgumentException("로또 번호는 중복되지 않는 정수 6개입니다.");
        }
    }
    ...
    public static void validateRangeOfBonusNumber(int bonusNumber) {
        if (!(bonusNumber >= LOWER_BOUND && bonusNumber <= UPPER_BOUND)) {
            throw new IllegalArgumentException("로또 번호는 1에서 45 사이의 정수여야 합니다.");
        }
    }
}